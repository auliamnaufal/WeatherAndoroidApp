...
import com.auliamnaufal.weatherapp.network.ApiConfig
...
class MainViewModel : ViewModel() {
    ...
    val forcastByCity = MutableLiveData<ForecastResponse>()
    ...
    fun forcastByCity(city: String) {
        ApiConfig.getApiService().forecastByCity(city).enqueue(object : Callback<ForecastResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    forcastByCity.postValue(response.body())
    ...
    fun getForcastByCity() : LiveData<ForecastResponse> = forcastByCity
